...
async function run() {
  ...
  for (const r of mockRestaurants) {
    const { data: restData, error: rErr } = await supabase.from('restaurants').insert({
      name: r.name,
      location: r.location,
      score: r.score,
      price_per_person: r.pricePerPerson,
      tags: r.tags,
      short_review: r.shortReview,
      is_curator_original: r.isCuratorOriginal,
      rarity: r.rarity
    }).select().single();

    if (rErr) { console.error('Rest err:', rErr); continue; }
    
    for (const v of r.visitRecords) {
      const { data: vData, error: vErr } = await supabase.from('visit_records').insert({
        restaurant_id: restData.id,
        reviewer_name: v.reviewerName,
        date: v.date,
        total_cost: v.totalCost,
        overall_experience: v.overallExperience,
        cover_image: v.coverImage,
        taste_score: r.score,
        value_score: r.score,
        env_score: r.score,
        unique_score: r.score,
        total_score: r.score
      }).select().single();

      if (vErr) { console.error('Vis err:', vErr); continue; }

      if (v.dishes && v.dishes.length > 0) {
        const dishInserts = v.dishes.map(d => ({
          visit_record_id: vData.id,
          name: d.name,
          review: d.review
        }));
        await supabase.from('dishes').insert(dishInserts);
      }
    }
  }
  console.log('Done!');
